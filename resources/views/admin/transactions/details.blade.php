            ...
            : URL::signedRoute('proof.file', ['encoded' => \App\Helper\ImageHelper::encodePath($purchase_detail->payment_proof_url)])
                    ...
                    @if($purchase_detail->status == 'paid')
                        <div class="action-group flex">
                            <a href="{{route('download.asset', ['id' =>$purchase_detail->id])}}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" download="{{$purchase_detail->name}}">Download</a>
                        </div>
                    @endif
