...
// ambil data json
$json = file_get_contents(__DIR__ . "../../../data/font.json");
// di decode dan juga menggunakna associative array
$data = json_decode($json, true);

// getting table user from database
$sql_for_user_icon = "select id, name from users";
...
// buat sql
$sql = "insert into user_details(user_id, bio, title, image_url) values(?, ?, ?, ?)";
...
for ($i = 0; $i < count($data); $i++) {
    ...
    if (!is_id_exist_in_user_details(getIdByName($data[$i]['penjual']))) {
        ...
        $bio = "Hello i'm " . $data[$i]['penjual'] . " and i love making font for fun.";
        $title = 'font maker';
        $image_url = $data[$i]['ppSeller'];
